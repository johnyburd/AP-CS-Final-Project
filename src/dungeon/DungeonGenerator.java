...
//                    dun[i][j] = 15;
    ...
    private void spawnBaddies(int numBaddies, int dungeonLevel)
    {
        for(int i = 0; i < dun.length; i++)
            ...
            {
                int a = (int)(Math.random() * 100);
                if((a < 10 && numBaddies > 0 && dun[i][j] == 0 && i != dun.length/2 && (j != 13 || j != 12 || j != 11)))
                {
                    numBaddies--;
                   // dun[i][j] = 15
                    Entity monster = new Entity(0.0045, 3.0, 10*dungeonLevel, 7 + dungeonLevel, Equipment.randomMonsterEquipment(dungeonLevel), (double)(i+0.5), (double)(j+0.5), textbox, player);
